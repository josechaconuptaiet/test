...
export const Inpux = forwardRef((props, ref) => {
  const { 
  ...
  return (
    <div 
      className={cn(
        'ac-inpux', `ac-inpux--${size}`, `ac-inpux--${variant}`, 
    ...
    >
      {showLabel && label && (
        <label className="ac-inpux-label" htmlFor={inputId}>
          {label} {required && <span className="ac-inpux-req">*</span>}
        </label>
      )}

      <div className={cn('ac-inpux-field', !!prefix && 'ac-has-prefix', (!!suffix || type === 'password') && 'ac-has-suffix')}>
        {prefix && <span className="ac-inpux-icon ac-prefix">{prefix}</span>}
        
        <input 
          ref={ref} id={inputId} name={name} type={inputType} className="ac-inpux-el" 
        ...
        {type === 'password' ? (
          <button type="button" className="ac-inpux-icon ac-suffix ac-pwd-toggle" onClick={togglePassword} tabIndex="-1">
            {suffix ? suffix : (
              showPassword ? (
